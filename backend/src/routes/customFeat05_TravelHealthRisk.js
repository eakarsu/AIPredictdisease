...
// User enters travel destination + medical history; AI assesses disease risks specific to destination + individual; recommends vaccines, prophylaxis
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPredictdisease / cf-travel-health-risk',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_travel_health_risk`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, aiRateLimiter, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Travel health risk'.
Context: User enters travel destination + medical history; AI assesses disease risks specific to destination + individual; recommends vaccines, prophylaxis
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-travel-health-risk/run error:', err);
