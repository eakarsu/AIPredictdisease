...
// Monitor treatment outcomes across cases; AI identifies most effective treatments for specific patient subgroups; updates recommendations
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPredictdisease / cf-treatment-efficacy-tracking',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_treatment_efficacy_tracking`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, aiRateLimiter, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Treatment efficacy tracking'.
Context: Monitor treatment outcomes across cases; AI identifies most effective treatments for specific patient subgroups; updates recommendations
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-treatment-efficacy-tracking/run error:', err);
