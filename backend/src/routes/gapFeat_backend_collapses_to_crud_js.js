...
// Gap Feature: Backend collapses to crud.js
// Backend collapses to crud.js — no modular per-domain routes
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPredictdisease / gap-backend-collapses-to-crud-js',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Backend collapses to crud.js'.\nProject: AIPredictdisease.\nContext: Backend collapses to crud.js — no modular per-domain routes\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['backend-collapses-to-crud-js', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'backend-collapses-to-crud-js', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
