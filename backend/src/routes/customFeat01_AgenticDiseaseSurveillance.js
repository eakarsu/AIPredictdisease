...
// Real-time monitoring of disease cases reported across regions; AI predicts outbreak hotspots; alerts public health officials with intervention recommendations
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPredictdisease / cf-agentic-disease-surveillance',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_disease_surveillance`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, aiRateLimiter, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic disease surveillance'.
Context: Real-time monitoring of disease cases reported across regions; AI predicts outbreak hotspots; alerts public health officials with intervention recommendations
Project: AIPredictdisease.
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-disease-surveillance/run error:', err);
