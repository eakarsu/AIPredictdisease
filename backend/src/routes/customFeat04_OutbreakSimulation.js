...
// Run "what-if" scenarios (new variant, vaccine uptake); AI predicts population outcomes; informs policy decisions
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPredictdisease / cf-outbreak-simulation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_outbreak_simulation`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, aiRateLimiter, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Outbreak simulation'.
Context: Run "what-if" scenarios (new variant, vaccine uptake); AI predicts population outcomes; informs policy decisions
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-outbreak-simulation/run error:', err);
