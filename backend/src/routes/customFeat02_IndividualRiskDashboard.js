...
// Patient demographics + medical history + genetic factors; AI generates personalized disease risk profile; recommends prevention/screening
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPredictdisease / cf-individual-risk-dashboard',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_individual_risk_dashboard`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, aiRateLimiter, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Individual risk dashboard'.
Context: Patient demographics + medical history + genetic factors; AI generates personalized disease risk profile; recommends prevention/screening
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-individual-risk-dashboard/run error:', err);
