...
// Custom Feature: Outbreak simulation
// Outbreak simulation: Run \"what-if\" scenarios (new variant, vaccine uptake); AI predicts population outcomes; informs policy decisions
import React, { useState } from 'react';

export default function CFOutbreakSimulationPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-outbreak-simulation/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Outbreak simulation</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Outbreak simulation: Run \"what-if\" scenarios (new variant, vaccine uptake); AI predicts population outcomes; informs policy decisions</p>
