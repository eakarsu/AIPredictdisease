...
// Gap (Non-AI): Backend collapses to crud.js
// Backend collapses to crud.js — no modular per-domain routes
import React, { useState } from 'react';

export default function GapBackendCollapsesToCrudJsPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-backend-collapses-to-crud-js/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Backend collapses to crud.js</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Backend collapses to crud.js — no modular per-domain routes</p>
