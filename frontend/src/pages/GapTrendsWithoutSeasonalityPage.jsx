...
// Gap (AI): Trends without '/seasonality
// Trends without /seasonality-predict (seasonal disease patterns)
import React, { useState } from 'react';

export default function GapTrendsWithoutSeasonalityPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-trends-without-seasonality/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Trends without '/seasonality</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Trends without /seasonality-predict (seasonal disease patterns)</p>
