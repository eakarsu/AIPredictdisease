...
// Gap (Non-AI): No webhooks for outbreak alerts
// No webhooks for outbreak alerts
import React, { useState } from 'react';

export default function GapNoWebhooksForOutbreakAlertsPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-webhooks-for-outbreak-alerts/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No webhooks for outbreak alerts</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No webhooks for outbreak alerts</p>
